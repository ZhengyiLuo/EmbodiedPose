...
class ChunkedRotGenerator:
    ...
    def __init__(self,
        ...
        for i in range(len(poses_2d)):
            assert poses_3d is None or poses_3d[i].shape[0] == poses_3d[
                i].shape[0]
            n_chunks = (poses_2d[i].shape[0] + chunk_length -
                        1) // chunk_length
            offset = (n_chunks * chunk_length - poses_2d[i].shape[0]) // 2
            bounds = np.arange(n_chunks + 1) * chunk_length - offset
            augment_vector = np.full(len(bounds - 1), False, dtype=bool)
            pairs += zip(np.repeat(i, len(bounds - 1)), bounds[:-1],
                         bounds[1:], augment_vector)
            if augment:
                pairs += zip(np.repeat(i, len(bounds - 1)), bounds[:-1],
                             bounds[1:], ~augment_vector)
        ...
        if poses_3d is not None:
            self.batch_3d = np.empty(
                (batch_size, chunk_length, poses_3d[0].shape[-2],
                 poses_3d[0].shape[-1]))
        self.batch_2d = np.empty(
            (batch_size, chunk_length + 2 * pad, poses_2d[0].shape[-2],
             poses_2d[0].shape[-1]))

        self.batch_rot = np.empty(
            (batch_size, chunk_length, poses_3d[0].shape[-2],
             poses_3d[0].shape[-1]))
    ...
    def next_epoch(self):
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):
                chunks = pairs[b_i * self.batch_size:(b_i + 1) *
                               self.batch_size]
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    ...
                    if flip:
                        # Flip 2D keypoints
                        self.batch_2d[i, :, :, 0] *= -1
                        self.batch_2d[i, :, self.kps_left +
                                      self.kps_right] = self.batch_2d[
                    ...
                    # 3D poses
                    if self.rot_3d is not None:
                        seq_3d = self.rot_3d[seq_i]
                        low_3d = max(start_3d, 0)
                        high_3d = min(end_3d, seq_3d.shape[0])
                        pad_left_3d = low_3d - start_3d
                        pad_right_3d = end_3d - high_3d
                        if pad_left_3d != 0 or pad_right_3d != 0:
                            self.batch_rot[i] = np.pad(
                                seq_3d[low_3d:high_3d],
                                ((pad_left_3d, pad_right_3d), (0, 0), (0, 0)),
                                'edge')
                        else:
                            self.batch_rot[i] = seq_3d[low_3d:high_3d]

                        if flip:
                            # Flip 3D joints
                            self.batch_rot[i, :, :, 0] *= -1
                            self.batch_rot[i, :, self.joints_left + self.joints_right] = \
                                    self.batch_rot[i, :, self.joints_right + self.joints_left]
                ...
                if self.poses_3d is None and self.cameras is None:
                    ...
                    yield None, self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], self.batch_rot[:len(chunks)]
                elif self.poses_3d is None:
                    yield self.batch_cam[:len(
                        chunks)], None, self.batch_2d[:len(chunks)]
                else:
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                if self.poses_3d is None and self.cameras is None:
                    ...
                    yield None, self.batch_3d[:len(
                        chunks)], self.batch_2d[:len(chunks)]
                elif self.poses_3d is None:
                    yield self.batch_cam[:len(
                        chunks)], None, self.batch_2d[:len(chunks)]
                else:
                    yield self.batch_cam[:len(chunks)], self.batch_3d[:len(
                        chunks)], self.batch_2d[:len(chunks)]
