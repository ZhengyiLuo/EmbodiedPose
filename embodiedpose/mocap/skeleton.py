from embodiedpose.utils.transformation import euler_matrix
        ...
        for line in content:
            ...
            if phase == 1:
                if cmd == 'begin':
                    ...
                    if swap_axes:
                        lastbone.dir[1], lastbone.dir[
                            2] = -lastbone.dir[2], lastbone.dir[1]
                elif cmd == 'length':
                    lastbone.len = float(line_words[1]) * self.len_scale
                elif cmd == 'axis':
                    args = [
                        math.radians(float(word)) for word in line_words[1:4]
                    ]
                    lastbone.orient = euler_matrix(*args, "sxyz")[:3, :3]
                    if swap_axes:
                        orient = lastbone.orient.copy()
                        lastbone.orient[1, :], lastbone.orient[
                            2, :] = -orient[2, :], orient[1, :]
                elif cmd == 'dof':
                    ...
                    lastbone.lb.append(
                        float(re.sub('[(]', ' ',
                                     line_words[1 - setting_limit])))
                    lastbone.ub.append(
                        float(re.sub('[)]', ' ',
                                     line_words[2 - setting_limit])))
                    setting_limit = True
