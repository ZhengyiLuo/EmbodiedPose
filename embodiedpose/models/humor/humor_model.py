...
from embodiedpose.models.humor.utils.transforms import (
    convert_to_rotmat, compute_world2aligned_mat,
...
class HumorModel(nn.Module):
    ...
    def __init__(
        ...
        self.data_names = [
            ...
            'pose_body', 'joints', 'joints_vel', 'contacts'
        ]
        ...
        #     for dname in self.data_names
        # ]
        self.output_dim_list = [3, 3, 3, 3, 63, 66, 66]
        # self.delta_output_dim_list = [
        #     data_dim(dname, rot_rep_size=ROT_REP_SIZE['mat'])
        #     for dname in self.data_names
        # ]
        self.delta_output_dim_list = [3, 3, 9, 3, 189, 66, 66]
        ...
        if use_vposer:
            expr_dir = './data/vposer/vposer_v1_0/'  # directory for the trained model along with the model code. obtain from https://smpl-x.is.tue.mpg.de/downloads
    ...
    def roll_out_single(
        ...
        # need to transform init input to local frame
        cur_input_dict, global_world2local_rot, global_world2local_trans = self.canonicalize_input(
            curr_global_dict, trans2joint)

        trans = cur_input_dict['trans']
        B, S, D = trans.size()
        ...
        if z_seq is not None:
            z_in = z_seq[:, 0]
        past_in = torch.cat(
            [cur_input_dict[k].reshape(B, -1) for k in self.data_names], dim=1)
        ...
        if return_z:
            z_out_seq = z_out
            cur_world_dict["z"] = z_out_seq

        if return_prior:
            pm = prior_out[0]
            pv = prior_out[1]
            return cur_world_dict, (pm, pv)
        else:
            return cur_world_dict

    def roll_out_single_global(
        self,
        init_input_dict,
        curr_global_dict,
        use_mean=False,
        ...
        global_world2local_trans = torch.cat(
            [
                ...
                torch.zeros((B, 1, 1)).to(trans),
            ],
            axis=2,
        )

        trans2joint = -(cur_input_dict['joints'].reshape(-1, 22, 3)[:, 0, :2])
        trans2joint = torch.cat(
        ...
        if z_seq is not None:
            z_in = z_seq[:, 0]
        past_in = torch.cat(
            [init_input_dict[k].reshape(B, -1) for k in self.data_names],
            dim=1)
        sample_out = self.sample_step(
            past_in,
            use_mean=use_mean,
        ...
        for k in cur_input_dict.keys():
            if k in x_pred_dict:
                ...
                S = cur_input_dict[k].shape[1]

                keep_frames = cur_input_dict[k][:, 1:, :]
                # keep_frames = cur_input_dict[k][:, 0, :]
                # print(keep_frames.size())
                ...
                if (k == "joints" and self.use_smpl_joint_inputs
                    ...
                    if S > 1:
                        ...
                        cur_input_dict[k] = x_pred_dict[k]
            else:
                del_keys.append(k)
        ...
        # compute the new world2aligned rot and translation
        root_orient_mat = x_pred_dict["root_orient"][:, 0, :].reshape(
            (B, 3, 3))
        world2aligned_rot = compute_world2aligned_mat(root_orient_mat)
        world2aligned_trans = torch.cat(
            [-x_pred_dict["trans"][:, 0, :2],
             torch.zeros((B, 1)).to(trans)],
            axis=1,
        )

        #
        # transform inputs to this (CURRENT) local frame (body pose is not affected) for next step
        #
    ...
    def step_action(self, past_in, action, output_delta=True):
        ...
        if output_delta:
            ...
            result_states = torch.cat(final_out_list, dim=2)

        result_states = result_states.reshape((B, -1))
    ...
    def direct_state(self, action, use_vposer=True):
        ...
        if use_vposer:
            ...
            pose_body = convert_to_rotmat(action[:, None, 9:135], rep="6d")
    ...
    def canonicalize_input_double(self,
                                  ...
                                  trans2joint=None):
        if split_input:
            past_global_in_dict = self.split_output(past_global_in_dict)
            next_global_in_dict = self.split_output(next_global_in_dict)
        ...
        # transform to local frame
        t_in_dict = dict()
        t_in_dict = self.apply_world2local_trans(world2aligned_trans,
                                                 world2aligned_rot,
                                                 trans2joint,
                                                 next_global_in_dict,
                                                 t_in_dict,
        ...
        next_out = []
        for k in self.data_names:
            next_out.append(t_in_dict[k].reshape(B, seq_len, -1))
