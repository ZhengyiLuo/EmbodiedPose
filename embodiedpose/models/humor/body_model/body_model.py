...
class BodyModel(nn.Module):
        ...
        if '.npz' in bm_path:
            ...
            if model_type == 'smplh':
                ...
                data_struct.shapedirs = np.concatenate(
                    [
                        data_struct.shapedirs,
                        np.zeros((V, D, SMPL.SHAPE_SPACE_DIM - B))
                    ],
    ...
    def forward(self,
        ...
        out_obj = self.bm(
            ...
            body_pose=pose_body,
            left_hand_pose=None if pose_hand is None else
            pose_hand[:, :(SMPLH.NUM_HAND_JOINTS * 3)],
            right_hand_pose=None
            if pose_hand is None else pose_hand[:,
                                                (SMPLH.NUM_HAND_JOINTS * 3):],
            transl=trans,
