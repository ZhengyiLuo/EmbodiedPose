...
from embodiedpose.models.humor.body_model.utils import SMPL_JOINTS

#
# For computing local body frame
#

# GLOB_DEVICE = torch.device(
...
XY_AXIS_GLOB = torch.DoubleTensor([[1.0, 1.0, 0.0]])
X_AXIS_GLOB = torch.DoubleTensor([[1.0, 0.0, 0.0]])


def compute_aligned_from_right(body_right):
    xy_axis = XY_AXIS_GLOB.to(body_right.device)
    x_axis = X_AXIS_GLOB.to(body_right.device)
    
    eps = 1e-8
    body_right_x_proj = body_right[:, 0:1] / (torch.norm(body_right[:, :2], dim=1, keepdim=True) + eps)
    body_right_x_proj = torch.clamp(body_right_x_proj, min=-1.0,
                                    max=1.0)  # avoid acos error
    ...
    world2aligned_aa = (world2aligned_axis /
                        (torch.norm(world2aligned_axis, dim=1, keepdim=True) +
                         eps)) * world2aligned_angle
...
def compute_world2aligned_mat(rot_pos):
    ...
    world2aligned_mat, world2aligned_aa = compute_aligned_from_right(
        body_right)
...
def convert_to_rotmat_res(pred_rot, rep='aa'):
    '''
    Converts rotation rep to rotation matrix based on the given type.
    pred_rot : B x T x N
    '''
    ...
    if rep == 'aa':
        ...
        B = pred_rot.shape[0]
        pred_rot = pred_rot + torch.eye(3)[None, :2].reshape(-1, 6).repeat(B, 1).to(pred_rot)
        pred_rot_mat = rot6d_to_rotmat(pred_rot)
    elif rep == '9d':
        pred_rot = pred_rot.reshape(-1, 9)
        # pred_rot = pred_rot + torch.eye(3)[None, :3].reshape(-1, 9).repeat(B, 1).to(pred_rot)
        pred_rot_mat = rot9d_to_rotmat(pred_rot)
    return pred_rot_mat.reshape((B, T, -1))
