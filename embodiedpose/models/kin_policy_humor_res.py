...
import copy
from copycat.utils.torch_ext import get_scheduler
...
from copycat.utils.torch_ext import isNpArray, dict_to_torch
import gc
...
class KinPolicyHumorRes(KinPolicy):
    def __init__(self, cfg, data_sample, device, dtype, agent, mode="train"):
        ...
        data_sample = {
            k: v.to(self.device).type(self.dtype) if isinstance(
    ...
    def forward(self, all_state, gather=True):
        if self.mode == "test" or not self.kin_net.use_rnn:
            ...
            s_ctx = all_state_vec.view(-1, self.max_episode_len,
                                       self.state_dim).transpose(
                                           0, 1).contiguous()
            ...
            action_mean_acc = torch.stack(action_mean_acc).transpose(
                0, 1).contiguous().view(-1, action_mean_acc[0].shape[-1])

            if gather:
                s_gather_indices = torch.LongTensor(
                    np.tile(self.indices[:, None],
                            (1, action_mean_acc.shape[-1]))).to(self.device)
                action_mean = torch.gather(action_mean_acc, 0,
                                           s_gather_indices)
            else:
    ...
    def init_context(self, data_dict, random_cam=False):
        ...
        self.action_log_std[:] = torch.tensor(
            self.cfg.policy_specs['log_std']).to(self.device)
    ...
    def scatter_vecs(self, vecs_list):
        ...
        for vec in vecs_list:
            ...
            s_gather_indices = torch.LongTensor(
                np.tile(self.indices[:, None],
                        (1, vec.shape[-1]))).to(self.device)
            s_vec = torch.zeros(
                (self.num_episode * self.max_episode_len, vec.shape[-1]),
                device=self.device)
    ...
    def update_supervised(self,
        ...
        rewards = seq_data[2]
        all_state_s, humor_target_vec_s, sim_humor_vec_s, rewards_s = self.scatter_vecs(
            [all_state, humor_target_vec, sim_humor_vec, rewards[:, None]])

        sim_humor_dict = self.kin_net.motion_prior.split_input(
            sim_humor_vec_s, convert_rots=False)
        humor_target_dict = self.kin_net.motion_prior.split_input(
            humor_target_vec_s[:, :sim_humor_vec_s.shape[1]],
            convert_rots=False)

        beta = humor_target_vec_s[:, -16:]
        points = humor_target_vec_s[:, sim_humor_vec_s.shape[1]:-16].reshape(
            humor_target_vec_s.shape[0], -1, 3)
        ...
        for epoch in pbar:
            ...
            next_global_out['trans'] = next_global_out['trans'].view(
                -1, self.max_episode_len, 3)
            next_global_out['root_orient'] = next_global_out[
                'root_orient'].view(-1, self.max_episode_len, 3, 3)
            next_global_out['pose_body'] = next_global_out['pose_body'].view(
                -1, self.max_episode_len, 21, 3, 3)
            next_global_out['joints'] = next_global_out['joints'].view(
                -1, self.max_episode_len, 22, 3)

            trans_vel, joints_vel, root_orient_vel = estimate_velocities(
                next_global_out['trans'],
                next_global_out['root_orient'],
                next_global_out['joints'],
                data_fps=30,
                aa_to_mat=False)
            ...
            next_global_out = {
                k: v.reshape(B, self.max_episode_len, -1)
                for k, v in next_global_out.items()
            }

            total_loss, loss_dict, loss_unweighted_dict = self.kin_net.compute_loss_seq(
                next_global_out,
                humor_target_dict,
                epoch=epoch,
                max_epoch=num_epoch)
            ...
            if self.use_tcn:
                gt_bpos = humor_target_dict['wbpos_cam'].reshape(
                    B, self.max_episode_len, 14, 3).clone()
                gt_bpos = gather_vecs([gt_bpos.reshape(-1, 14 * 3)],
                                      self.indices)[0].reshape(-1, 14, 3)
                gt_traj = gt_bpos[..., 7:8, :]

                gt_body_pos = gt_bpos - gt_traj
                tcn_input = all_state[:, (
                    self.kin_net.is_root_obs == 3)[0]].reshape(
                        -1, self.kin_net.num_context, 12,
                        self.kin_net.traj_in_features).float()
                ...
                for tcn_input_idx in torch.split(torch.randperm(tcn_input.shape[0]), 1024, 0):
                    ...
                    if self.kin_net.model_specs.get("tcn_3dpos", False):
                        ...
                        kp_2d_feats_local[..., 2:] = tcn_input_chunk[
                            ..., 2:] - tcn_input_chunk[..., 7:8, 2:]
                        if self.kin_net.model_specs.get("tcn_body2d", False):
                            pred_body_pos = self.kin_net.tcn_body_1f(
                                kp_2d_feats_local[..., :2])
                        else:
                            pred_body_pos = self.kin_net.tcn_body_1f(
                                kp_2d_feats_local)
                    ...
                    root_mpjpe = torch.norm(gt_traj_chunk.reshape(-1, 3) -
                                            pred_traj.reshape(-1, 3),
                                            dim=1)

                    body_mpjpe = torch.norm(
                        gt_body_pos_chunk.reshape(-1, 14, 3) -
                        pred_body_pos.reshape(-1, 14, 3),
                        dim=2).mean(-1)
                    ...
                    root_mpjpe_acc.append(
                        root_mpjpe.mean().detach().cpu().item())
                    body_mpjpe_acc.append(
                        body_mpjpe.mean().detach().cpu().item())

                loss_dict['root_mpjpe'] = np.mean(root_mpjpe_acc)
                loss_dict['body_mpjpe'] = np.mean(body_mpjpe_acc)

            pbar.set_description_str(
                f"Per-step loss: {total_loss.cpu().detach().numpy():.3f} [{ {k: f'{v:.3f}'  for k, v in loss_dict.items()}}] lr: {self.scheduler.get_last_lr()[0]:.5f} | num_s {num_samples} | max_ep {self.max_episode_len}"
            )
            gc.collect()
