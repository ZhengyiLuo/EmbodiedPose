...
class SMPLModel():
    ...
    """
        with open(model_path, 'rb') as f:
            ...
            self.shapedirs = np.array(params['shapedirs']).astype(np.float64)
            self.faces = params['f']
    ...
    def forward(self, input_vec):
        pose, beta, trans = input_vec[:72], input_vec[72:82], input_vec[82:85]
        #         # how beta affect body shape
...
smpl = SMPLModel('/hdd/zen/data/SMPL/smpl_models/SMPL_NEUTRAL.pkl')
np.random.seed(9608)
pose = (np.random.rand(*smpl.pose_shape) - 0.5) * 0.4
beta = (np.random.rand(*smpl.beta_shape) - 0.5) * 0.06
trans = np.zeros(smpl.trans_shape)

input_vec = np.concatenate([pose.reshape(72), beta, trans], axis=0)
import time
t_s = time.time()
out = smpl.forward(input_vec)
dt = time.time() - t_s
print(dt)

from autograd import elementwise_grad as egrad
smpl_fk = egrad(smpl.forward)
