...
class LoggerRL:
    ...
    def end_episode(self, env):
        ...
        self.min_episode_reward = min(self.min_episode_reward,
                                      self.episode_reward)
        self.max_episode_reward = max(self.max_episode_reward,
                                      self.episode_reward)
    ...
    def merge(cls, logger_list):
        ...
        logger.max_episode_reward = max(
            [x.max_episode_reward for x in logger_list])
        logger.min_episode_reward = max(
            [x.min_episode_reward for x in logger_list])
