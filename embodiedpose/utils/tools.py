...
from embodiedpose.utils.math_utils import *
import glfw
import cv2
...
def out_dir():
    return path.abspath(
        path.join(path.dirname(path.abspath(__file__)), '../out'))
...
def save_screen_shots(window, file_name, transparent=False):
    ...
    image = cv2.cvtColor(
        np.array(image),
        cv2.COLOR_RGB2BGRA if transparent else cv2.COLOR_RGB2BGR)
    if transparent:
        image[np.all(image >= [240, 240, 240, 240],
...
def get_chunk_with_overlap(num_frames, window_size=80, overlap=10):
    ...
    chunk_idxes = view_as_windows(np.array(range(num_frames)),
                                  window_size,
                                  step=step)
    chunk_supp = np.linspace(num_frames - window_size,
                             num_frames - 1,
                             num=window_size).astype(int)
    chunk_idxes = np.concatenate((chunk_idxes, chunk_supp[None, ]))
    last_chunk = chunk_idxes[-1][:step][-1] - chunk_idxes[-2][:step][-1] + int(
        overlap / 2)
    chunck_selects = get_chunk_selects(chunk_idxes,
                                       last_chunk,
...
def fix_height(expert):
    ...
    if ground_pene < -0.15:
        print(expert_meta['seq_name'], "negative sequence invalid for copycat",
              ground_pene)
        return None
    return new_expert