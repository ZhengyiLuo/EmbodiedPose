...
class Config:
    def __init__(self, action, cfg_id, create_dirs=False):
        ...
        # exit(0)
        cfg_path = f"cfg/**/{cfg_id}.yml"
        files = glob.glob(cfg_path, recursive=True)
        assert (len(files) == 1)
        cfg_name = files[0]

        self.yam_data = cfg = yaml.load(open(cfg_name, 'r'),
        ...
        self.meta = yaml.load(open(
            '%s/meta/%s.yml' % (self.data_dir, self.meta_id), 'r'),
                              Loader=yaml.FullLoader)
        self.object = self.meta['object'][action]
        self.mujoco_model_file = osp.join(os.getcwd(), "assets",
                                          "mujoco_models", cfg['model_id'],
                                          action, "humanoid.xml")
        ...
        self.space_model_file = 'results/occ_enc/%s/models/iter_%04d.p' % (
            cfg['space_model'], cfg['space_feat_iter'])
