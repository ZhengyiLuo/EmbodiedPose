...
from embodiedpose.utils.torch_geometry_transforms import *
from embodiedpose.utils.transformation import quaternion_slerp
from embodiedpose.utils.torch_utils import quaternion_matrix_batch
...
def vertizalize_smpl_root(poses, root_vec=[np.pi / 2, 0, 0]):
    ...
    target_mat = angle_axis_to_rotation_matrix(
        torch.tensor([root_vec],
                     dtype=poses.dtype).to(device))[:, :3, :3].to(device)
    org_mats = angle_axis_to_rotation_matrix(
        poses[:, :3])[:, :3, :3].to(device)
    ...
    zeros = torch.zeros((res_root_mat.shape[0], res_root_mat.shape[1], 1),
                        dtype=res_root_mat.dtype).to(device)
