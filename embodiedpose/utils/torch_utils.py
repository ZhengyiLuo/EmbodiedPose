...
def quaternion_from_euler(ai, aj, ak, axes='sxyz'):
    ...
    q = torch.tensor([1.0, 0.0, 0.0, 0.0] * B, dtype=dtype,
                     device=device).view(B, 4)
    if repetition:
        q[:, 0] = cj * (cc - ss)
        q[:, i] = cj * (cs + sc)
        q[:, j] = sj * (cc + ss)
...
def rotation_from_quaternion_batch(_q, separate=False):
    ...
    device = q.device
    dtype = q.dtype
    zero_axis = torch.tensor([1.0, 0.0, 0.0] * B, dtype=dtype,
                             device=device).view(B, 3)
    ...
    axis = torch.where(
        cond.unsqueeze(1).repeat(1, 3), zero_axis,
...
def quaternion_about_axis(angle, axis):
    device = angle.device
    dtype = angle.dtype
    q = torch.tensor([0.0, axis[0], axis[1], axis[2]],
                     dtype=dtype,
                     device=device)
...
def quaternion_about_axis_batch(angle, axis):
    ...
    q_change = q[qlen > _EPS, :] * torch.sin(
        angle[qlen > _EPS, :] / 2.0) / qlen[qlen > _EPS].view(-1, 1)
    q_res = q.clone()
...
def quat_from_expmap_batch(e):
    ...
    axis[angle < 1e-8] = torch.tensor([1.0, 0.0, 0.0],
                                      dtype=dtype,
                                      device=device)
...
if __name__ == "__main__":
    ...
    from embodiedpose.utils.math_utils import quaternion_matrix as quaternion_matrix_np
    from embodiedpose.utils.transformation import quaternion_inverse as quaternion_inverse_np
    from embodiedpose.utils.math_utils import transform_vec as transform_vec_np
    from embodiedpose.utils.math_utils import quat_from_expmap as quat_from_expmap_np
    from embodiedpose.utils.transformation import quaternion_multiply as quaternion_multiply_np
    from embodiedpose.utils.math_utils import get_heading_q as get_heading_q_np
    from embodiedpose.utils.math_utils import get_heading as get_heading_np
    from embodiedpose.utils.transformation import rotation_from_quaternion as rotation_from_quaternion_np
    from embodiedpose.utils.math_utils import quat_mul_vec as quat_mul_vec_np
    from embodiedpose.utils.math_utils import de_heading as de_heading_np

    devices = [torch.device('cuda', index=0), torch.device('cpu')]

    for d in devices:
        ex_quat = torch.tensor([1.522, 0.560, 0.161, 0.623],
                               dtype=torch.float64)
        ex_quat_batch = torch.tensor(
            [[0.785, 1.360, 0.197, 0.464], [1.777, 3.363, 0.363, 0.363],
