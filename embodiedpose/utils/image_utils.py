...
def assemble_videos(videos,
    ...
    canvas = np.zeros(
        (y_shape * y_grid_num, x_shape * x_grid_num, 3)).astype(np.uint8)

    out = cv2.VideoWriter(out_file_name, cv2.VideoWriter_fourcc(*'FMP4'), 30,
    ...
    for i in range(len(videos[0])):
        for x in range(x_grid_num):
            for y in range(y_grid_num):
                ...
                canvas[y_shape * y:y_shape * (y + 1),
                       x_shape * x:x_shape * (x + 1), :] = curr_image
                cv2.putText(canvas, curr_discription,
                            (x_shape * x, y_shape * y + 20), 2, 0.5,
                            text_color)
        out.write(canvas)
    out.release()
...
def write_frames_to_video(frames,
                          out_file_name="output.mp4",
                          frame_rate=30,
                          add_text=None,
                          text_color=(255, 255, 255)):
    y_shape, x_shape, _ = frames[0].shape
    out = cv2.VideoWriter(out_file_name, cv2.VideoWriter_fourcc(*'FMP4'),
                          frame_rate, (x_shape, y_shape))
