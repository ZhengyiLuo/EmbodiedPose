...
parser.add_argument(
    '--data_dir',
    action='store_true',
    default="/hdd/zen/data/Reallite/contextegopose/ReliveDataset/")
...
for file in fpv_files:
    ...
    cmd = [
        'ffmpeg', '-y', '-i', file, '-vf',
        'crop=%d:%d:%d:%d' % (c_size, c_size, v_width // 2 - c_size // 2,
                              v_height // 2 - c_size // 2), '-c:v', 'libx264',
        '-crf', '15', '-pix_fmt', 'yuv420p', crop_file
    ]
    subprocess.call(cmd)

    cmd = [
        'ffmpeg', '-y', '-i', crop_file, '-vf',
        'scale=%d:-1' % s_size, '-c:v', 'libx264', '-crf', '15', '-pix_fmt',
        'yuv420p', scale_file
    ]
    subprocess.call(cmd)

    cmd = [
        'ffmpeg', '-i', scale_file, '-r', '30', '-start_number', '0',
        os.path.join(frame_dir, '%05d.png')
    ]
    subprocess.call(cmd)
    if args.wild:
        cmd = [
            'ffmpeg', '-i', file, '-r', '60', '-start_number', '0',
            os.path.join(frame_slam_dir, '%05d.png')
        ]
        subprocess.call(cmd)
