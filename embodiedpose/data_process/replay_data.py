...
from embodiedpose.envs.common.mjviewer import MjViewer
import pickle
import glob
import argparse
import glfw
import yaml
import cv2
from embodiedpose.utils.transformation import quaternion_from_euler, quaternion_multiply
...
if args.meta_id is not None:
    meta_file = os.path.expanduser('%s/meta/%s.yml' %
                                   (args.dataset_path, args.meta_id))
...
model_file = 'assets/mujoco_models/%s/%s/humanoid_vis_single.xml' % (
    args.model_id, args.action)
print(model_file)
...
take_folders = glob.glob(
    os.path.expanduser('%s/fpv_frames/%s_*' %
                       (args.dataset_path, args.mocap_id)))
take_folders.sort()
takes = []
...
print(takes)
bvh_files = glob.glob(
    os.path.expanduser('%s/bvh_raw/*actor.bvh' % args.dataset_path))
...
def update_fpv():
    print(
        'take: %s  fr: %d  im_fr: %d  offset: %d  fr_boundary: (%d, %d)  dz: %.3f'
        % (takes[take_ind], fr, fr + im_offset, im_offset, max(
            -im_offset, 0), min(qpos_traj.shape[0],
                                len(images) - im_offset - 1), offset_z))
...
def load_take():
    ...
    take = takes[take_ind]
    print(take)
    im_offset = 0 if not take in meta['video_mocap_sync'].keys(
    ) else meta['video_mocap_sync'][take][0]
    #im_offset = 0 if args.meta_id is None else meta['video_mocap_sync'][take][0]
    fr = max(0, -im_offset)
    traj_file = os.path.expanduser('%s/traj/%s_traj.p' %
                                   (args.dataset_path, take))
    ...
    '''
    frame_folder = os.path.expanduser('%s/fpv_frames/%s' %
                                      (args.dataset_path, take))
    frame_files = glob.glob(os.path.join(frame_folder, '*.png'))
    frame_files.sort()
    images = [cv2.imread(file) for file in frame_files]
    print('traj len: %d,  image num: %d,  dz: %.3f' %
          (qpos_traj.shape[0], len(images), offset_z))
...
while not stop:
    ...
    if t >= math.floor(T):
        if not reverse and fr + im_offset < len(
                images) - 1 and fr < qpos_traj.shape[0] - 1:
            fr += 1
            update_all()
